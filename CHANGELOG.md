...
### Исправлено — HTTP 413 на больших вебхуках

Pyrus изредка присылает вебхуки размером больше 2 МБ (большие задачи
с длинной историей комментариев). Встроенный вебхук-сервер молча
отклонял их: у aiohttp лимит тела запроса по умолчанию — 1 МиБ,
и такие события терялись после трёх ретраев Pyrus.

- Лимит приёма вебхуков поднят до **16 МиБ** по умолчанию.
- Новый параметр **`max_request_size`** (в байтах) у
  `dp.start_webhook(...)` и `create_app(...)` — если нужно больше:
...
- Превышение лимита теперь пишется в лог `aiopyrus.webhook` уровнем
  ERROR (с фактическим `Content-Length` и подсказкой), а не теряется
  молча.

> ⚠️ Если перед ботом стоит nginx / Angie — поднимите лимит и там:
> `client_max_body_size 16m;` (у nginx по умолчанию 1 МБ, и тогда 413
> вернёт прокси, не дойдя до бота).

Поллинга (`start_polling` / `start_inbox_polling`) это не касается —
там данные забирает сам клиент через API.
